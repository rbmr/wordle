...
from wordle.solver import load_words, words_to_arr, get_word, pick_best_word, process_result
...
def get_guess(suggested_word: str, n_chars: int = 5) -> Optional[str]:
    while True:
        guess = Prompt.ask(
            f"Enter your guess (or 'q' to quit)",
        ...
        if len(guess) != n_chars:
            console.print(f"Error: Guess must be {n_chars} letters long. Try again.", style="bold")
            continue
        if not guess.isalpha():
            console.print("Error: Guess must only contain letters. Try again.", style="bold")
            continue
...
def get_response(n_chars: int = 5) -> str | None:
    ...
    prompt_text.append("\nEnter the response:\n")
    prompt_text.append(" (")
    prompt_text.append("G", style=f"bold")
    prompt_text.append(" = Green, ")
    prompt_text.append("Y", style=f"bold")
    prompt_text.append(" = Yellow, ")
    prompt_text.append("B", style=f"bold")
    ...
    while True:
        ...
        if resp == "Q":
            ...
            return None
        if len(resp) != n_chars:
            console.print(f"Error: Response must be {n_chars} letters long. Try again.", style="bold")
            continue
        if not set(resp).issubset({"G", "Y", "B"}):
            console.print("Error: Response must only contain G, Y, or B. Try again.", style="bold")
            continue
...
def get_words_list(words_file: Optional[Path] = None, n_chars: int = 5) -> list[str]:
    if words_file is None:
        ...
        try:
            ...
            default_words_file = importlib.resources.files("wordle").joinpath("words.txt")
            ...
            console.print("Error: Default 'words.txt' not found within the package.", style="bold")
...
def main(
    ...
    console.print(f"Starting Wordle Solver CLI (Loaded {len(words_list)} words)", style=f"bold")
    while True:
        n_remaining = len(words_arr)
        if n_remaining == 0:
            console.print("\n[bold]No possible words match the given clues. Check your responses.[/bold]")
            break
        ...
        if n_remaining > max_words:
            words_text.append(f" and {n_remaining - max_words} more...", style=f"italic dim")
